...
const AddEditLanding = () => {
  ...
  const handleSaveEvent = async () => {
    ...
    try {
      ...
      if (id === "b1") {
        const response = await apiServices.saveLanding(newEvent);
        ...
        await apiServices.updateLanding(id, newEvent);
  ...
  return (
    <>
      <Paper elevation={3} sx={{ p: 2, maxWidth: '80%', margin: '150px auto' }}>
        <Typography variant="h4">{id !== "b1" ? 'Edit' : 'Add'} Photo</Typography>
        <Box my={1}>
          <TextField
            fullWidth
            label="Movie Title"
        ...
        <Box my={2}>
          <Button variant="contained"  style={{margin:"5px"}} onClick={handleSaveEvent}>
            {id !== "b1" ? 'Update Photos' : 'Save Photo'}
          </Button>
        </Box>
      </Paper>
      <Snackbar
